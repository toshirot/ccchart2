    ...
    preProcessing: function (op, callback) {
      ...
      */
      ...
      if (this.type === 'scatter' || this.type === 'heatmap') {
        ...
        //maxYを上に広げる倍率 2018-04-13
        this.maxYRange = this.util.setConfigNum(this, 'maxYRange', this.op.config.maxYRange, this.gcf.maxYRange, 0);
        //minYを下に広げる倍率 2018-04-13
        this.minYRange = this.util.setConfigNum(this, 'minYRange', this.op.config.minYRange, this.gcf.minYRange, 0);

        if (this.maxYRange!==0){
          this.maxY=Math.ceil(+((this.maxY)*this.maxYRange)*100)/100;
        }
        if (this.minYRange!==0){
          this.minY=Math.floor(+((this.minY)*this.minYRange)*100)/100;
        }
      ...
      //Y軸目盛の間隔 キリ番化したいときなどに使います
      //  

      this.niceRound =
        this.op.config.niceRound || this.gcf.niceRound || "yes";
      if(this.niceRound==='yes'){
      /*
              this._yGapValue=(this.maxY-this.minY)/this.axisXLen;

              //水平目盛線1本当たりのラベル値
              this.yGapValue = +this.util.round00(this._yGapValue ,-((''+parseInt(this._yGapValue )).length-2));
              this.maxY=this.yGapValue*this.axisXLen+this.minY;
      */
        this.yGapValue = (this.maxY - this.minY) / this.axisXLen;
        //for ccy とりあえず8桁 2018-04-08
        this.yGapValue=((this.yGapValue).toFixed(8));
      //  this.yGapValue =this.util.marume_round(this.yGapValue , 4)
      } else {
        //水平目盛線1本当たりのラベル値
        this.yGapValue = +(((this.maxY - this.minY) / this.axisXLen).toFixed(8));
      }
...
*/
    ...
    drawAxisY: function () {
      ...
      //垂直目盛スキップ線の幅 //デフォルト3を1に変更した 2017/8/20
       this.axisYSkipWidth = this.util.setNumberOfConfigVal(this, 'axisYSkipWidth', 1);
        ...
        left += this.xGap, count++
          ...
          if(this.type !=='pie'){
            if((this.useFirstToColName && this.onlyChart==='no')||
              ...
              if(this.xScaleSkip!==0){
                var _rightAlign = true;//スキップの項目名右揃え デフォルトは左揃え あとでオプション化する
                var _rightAlignCount=count;
                if(_rightAlign)_rightAlignCount=count+1;

                if(_rightAlignCount% this.xScaleSkip === 0){
    ...
    drawYscale: function(top, count){
      ...
      if(this.type === 'stacked%'){
        ...
        //for ccy 呼び値に合わせる 2018-04-08
        //出力用で丸めるthis._wkYScaleと加算続けるthis.wkYScaleを分ける
        // this._wkYScale=window.toFixBPDLen(this.wkYScale); <読みにくい

        this._wkYScale=(+this.wkYScale).toFixed(4);
        if(+this._wkYScale===0){
          this._wkYScale='0';// 0.000000 は 0にする
        }

        if(this.maxY>100){
          val = this.util.addComma(this, this._wkYScale, yScaleDecimal);
        } else {
          val = this._wkYScale;
        }
      ...
      //出力用 wkYScaleStrが undefinedなら '' とする 2018-04-09
      if(!val)this.wkYScaleStr='';
      ...
      this.wkYScale = +this.wkYScale //parseFloat(this.wkYScale);//change type to number
      this.wkYScale += +this.yGapValue;
      //for ccy 小数4桁にしてしまうと 5桁以下が0になってしまう 2018-04-08
     // this.wkYScale = this.util.marume_round(this.wkYScale , 4)
    ...
    drawXLine: function () {
      ...
      for (var i = 0; i < op.length; i++) {
        ...
        var shdw = (this.shadows) ? this.shadows.xline || this.shadows.all || ['#444', 2, 2, 2] : '';
      ...
      for (var k = 0; k < this.dataRowLen; k++) {
        ...
        for (var l = 0; l < this.data[k].length; l++) {
          ...
          if (this.type === 'area'){
            ...
            if(k===0){
              //k===0 つまり row0で y値===chartBottomが現れたら一つ前のpらathをy=0へ下す
              if(y===this.chartBottom){
              　　this.ctx.lineTo(x-this.xGap, this.chartBottom);
              } else {
                this.ctx.lineTo(x, y);
              }
            } else if(k===1){
              //k===1 つまり row1で y値<chartBottomが現れたら
              if(y<this.chartBottom && !depthFlg){
                //最初のひとつ!depthFlgをchartBottomから立ち上げる
                this.ctx.lineTo(x, this.chartBottom);
                this.ctx.lineTo(x, y);
                depthFlg=true;
              } else {
                this.ctx.lineTo(x, y);
    ...
    drawCandle: function(){
      ...
      for (var k = 0; k <= that.dataColLen; k++) {//date
          ...
          open  = +this.util.marume_round(_data[0][k], 4)//parseInt(_data[0][k],10);
          hight = +this.util.marume_round(_data[1][k], 4)//+_data[1][k] //parseInt(_data[1][k],10);
          low   = +this.util.marume_round(_data[2][k], 4)//+_data[2][k] //parseInt(_data[2][k],10);
          close = +this.util.marume_round(_data[3][k], 4)//+_data[3][k]//parseInt(_data[3][k],10);

          //console.log(2222,this.type, _data[0][k], _data[1][k], _data[2][k],_data[3][k])
          //console.log(22221,this.type, open, hight, low,close)
          upDown = close - open;
          if (upDown>0){
          ...
          //y_line_hight=this.util.marume_round (y_line_hight, 4);
          //y_line_low=this.util.marume_round (y_line_low, 4);
          ...
          if(upDown===0)y_bar_close=1;

          //console.log(k, upDown,'o ',open,' h ',hight,' l ',low,' c ',close)
          /*
        console.log(k, '-----',
          x_bar, y_bar_open,
          candleBoxWidth,
...
/*
    ...
    util:{
      marume_round: function (num, keta){ //this.util.marume_round (num, keta)
        var _keta = Math.pow(10, keta);
        var _num = num*_keta;
        return Math.round(_num)/_keta;//四捨五入
      },
