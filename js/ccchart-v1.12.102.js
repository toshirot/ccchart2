    ...
    return {
        ...
        init: function init(id, op, callback) {
          ...
          + 'spell, or single-byte and double-byte is different'
            ...
            let _curr_classNames =this.canvas.getAttribute('class');

            let _classNames = _curr_classNames?' '+_curr_classNames:'';
            ...
            this.ctx =
            this.cxs[this.id] =this.canvas.getContext('2d');
            ...
            let hybrid = document.getElementById('-ccchart-css-hybrid');
            if (hybrid) {
                // 共通のキャンバスで書き換えるflipやws時にgroupbox配下をリセットする 20130413 thanx nohinaさん
                let cssgbox = document.getElementById('-ccchart-css-groupbox');
                let cssg = document.getElementById('-ccchart-css-group-'+this.id);
...
this.paddingTopDefault = this.paddingTopDefault - 15;
                ...
                if (this.yScalePercent === 'no') {this.paddingLeftDefault = 70;
}
...
_setRoundedUpMax(this, 'maxY', 'roundedUpMaxY');
                ...
                if (this.xScaleDecimal !== 'yes') {_setRoundedUpMax(this, 'maxX', 'roundedUpMaxX');
}
                ...
                function _getSatterMin(it, index) { // index0　is X, 1 is Y
                    let wk = it.util.hCopy(it.data[index]); // ハードコピー　data[1]はY
            ...
            function _setRoundedUpMax(that, max, roundedUp) {
                ...
                if (that.yScalePercent === 'no') {
                    let _rumy = Math.pow(10, ('' + that[max]).split('.')[0].length - 2);
...
this._ondrew = this._ondrew_old = function(that) {
        ...
        get: function(url, fnc, async) {
            ...
            req.onreadystatechange = function(evt) {
                if (req.readyState == 4 && req.status == 200) {
                    ...
                    let res = decodeURIComponent(req.responseText);
                    if (fnc) fnc(res);
                }
            };
            req.send();
        ...
        draw: function(op) {
            ...
            if (this._addsFlg === 0 || this._addsFlg === 1) {
                ...
                if (this.onlyChart === 'no' || this.onlyChartWidthTitle === 'yes') {this.drawTitle();
}
        ...
        drawGradient: function() {
            let bg = this.bgGradient;
            let y = (bg.direction==='vertical')?this.height:0;
            let x = (bg.direction==='horizontal')?this.width:0;
        ...
        drawAxisX: function() {
            ...
            let _axisXWidth = this.axisXWidth;
            ...
            function _doit( top, count) {
                ...
                // Y方向の垂直軸ラベル描画へ
                if (that.type !=='pie') {if (that.onlyChart==='no') {
        ...
        drawAxisY: function() {
            ...
            let _axisYWidth = this.axisYWidth;
            for (
                let left = this.chartLeft, count = 0;
        ...
        drawXscale: function(left, count) {
            ...
            this.ctx.save();
            let xScaleDecimal = this.xScaleDecimal;
            let xScaleColor =
        ...
        this.util.setConfigNum(this, 'xScaleRotate', this.op.config.xScaleRotate, this.gcf.xScaleRotate, 0);
            ...
            if (this.type==='scatter'||this.type==='heatmap') {
                var val = '', percent = '';
                if (left >= this.chartRight) {
                    this.wkXScale = this.maxX;
                }// 小手先修正あとで再考
        ...
        (this.type==='scatter'||this.type==='heatmap')?0:this.xGap/2 - xOffset;
            ...
            if (count===0) {
                let text = (this.colNamesTitle==='')?'':'(' + this.colNamesTitle + ')';
                let x = this.chartRight + tcXOffset;
        ...
        drawYscale: function(top, count) {
            ...
            let yScaleDecimal = this.yScaleDecimal;
            let yScaleColor =
        ...
        this.util.setConfigNum(this, 'yScaleRotate', this.op.config.yScaleRotate, this.gcf.yScaleRotate, 0);
            ...
            // Y軸反転 DESCはデフォルト降順
            let expression = top <= this.chartTop;
            if (this.type === 'line' || this.type === 'bezi2' || this.type === 'bezi') {
                if (this.yScaleOrder === 'ASC') {
                    // lineだけY軸目盛昇順可能　yScaleOrder
            ...
            if (count===0) {
                if (this.type==='scatter'||this.type==='heatmap') {
                    let text = (this.rowNamesTitle==='')?'':'(' + this.rowNamesTitle + ')';
                    let x = this.chartLeft + -10;
                    let y = this.chartTop + (this.chartHeight/2);
                    if (yScaleFont) this.ctx.font = yScaleFont;
                    this.ctx.fillStyle = yScaleColor;
                    this.ctx.textAlign = yScaleAlign;
        ...
        drawTitle: function() {
            ...
            this.ctx.save();
            let title = this.title || this.gcf.title || '';
            let titleFont =
        ...
        this.textColors.title || '#ccc';
            let offsetX = this.util.setNumberOfConfigVal(this, 'titleX', this.width/2);
            // adjust titleX by the align anchor point.
            let titleX = this.util.ajustTitlesX(this, titleTextAlign, offsetX);
            // this.op.config.titleX or this.gcf.titleX or this.width/2;
            let titleY = this.util.setNumberOfConfigVal(this, 'titleY', 38);
        ...
        drawSubTitle: function() {
            ...
            let subTitleFont = this.op.config.subTitleFont || this.gcf.subTitleFont;
            let subTitleTextAlign = this.op.config.subTitleTextAlign || this.gcf.subTitleTextAlign || 'center';
            let subTitleColor =
        this.op.config.subTitleColor || this.gcf.subTitleColor ||
        this.textColor ||
        this.textColors.all||
        this.textColors.subTitle || '#ddd';
            let offsetX = this.util.setNumberOfConfigVal(this, 'subTitleX', this.width/2);
            // adjust titleX by the align anchor point.
            let subTitleX = this.util.ajustTitlesX(this, subTitleTextAlign, offsetX);
            // this.op.config.subTitleX or this.gcf.subTitleX or this.width/2;
            let subTitleY = this.util.setNumberOfConfigVal(this, 'subTitleY', 55);
        ...
        drowHanrei: function() {
            // 凡例出力
            this.ctx.save();
            let len = this.hanreiNames.length;
            let xOffset =
        ...
        this.op.config.hanreiLineHeight || this.gcf.hanreiLineHeight || ((len < 10)?20:(len < 20)?14:8);// fixed at v1.10.3 thanx 舩原一平さん
            let fontsize = (len < 10)?12:(len < 20)?9:6;
            let font =
        this.op.config.hanreiFont || this.gcf.hanreiFont || '100 '+fontsize+'px \'Arial\'';
            ...
            let hanreiOrder = this.op.config.hanreiOrder || this.gcf.hanreiOrder || 'nomal';
        ...
        '#ccc';
            let align = this.op.config.hanreiAlign || this.gcf.hanreiAlign || 'left';
            let shdw = (this.shadows)?this.shadows.hanrei||this.shadows.all||['#222', 5, 5, 5]:'';
        ...
        drawUnit: function(unit) {
            ...
            let unitTitle = navigator.language==='ja'?'単位:':'';
            let left =
        ...
        _drawVals: function(op) {
            ...
            let xOffset = it.valXOffset = it.op.config.valXOffset || op.xoff || it.gcf.valXOffset || 10;
            let yOffset = it.valYOffset = it.op.config.valYOffset || op.yoff || it.gcf.valYOffset || -20;
            let font = it.valFont = it.op.config.valFont || op.font || it.gcf.valFont;
        ...
        it.valColor.length > 0) ?
            ...
            val = op.val;
            let percent = this.util.mkPercentVal(it, val, k, l);
        ...
        drawMarkers: function(op) {
            ...
            let markerWidth = op.markerWidth || that.markerWidth;
            let colorSet = op.colorSet || that.colorSet;
            let wkOffset = (that.type==='scatter'||that.type==='heatmap') ? 0 : that.xGap / 2;
            let colorIndex = 0;
          ...
          document.querySelector('#-ccchart-css-group-' + that.id);
            ...
            if (that.type === 'scatter') {
                ...
                let _initX = x; // 初期 left
                let barLeft =0; // 各バーのleft位置 の初期値

                for (var k = 0; k < that.dataRowLen; k++) {// bar
                    for (var l = 0; l < that.data[k].length; l++) {// col
                        let _bityousei = ((that.data[k][l]>0)?that.barWidth/2:-that.barWidth/2);
              ...
              data[k][l] * that.unitH;
                ...
                let cdata=[];
                let _xoffset = that.paddingLeft - that.minX * that.unitW;
                let _yoffset = that.chartBottom + that.minY * that.unitH;
                ...
                let colorGrad = op.colorGrad;
                let colord=that.util.hm.coloring(that._hm_grayGrad, colorGrad);
            ...
            function _drawmarkers(x, y, row, col, data, colorSet, colorIndex) {
                let curdata = data[row][col];
                if (curdata===''||curdata===undefined||isNaN(curdata)) {
                    return;
                }
                that.ctx.beginPath();
                that.ctx.fillStyle = colorSet[colorIndex];
                let scatterX = (that.type==='scatter'||that.type==='heatmap') ? ((data[0])?(data[0][col]):'') : '';
                let scatterY = (that.type==='scatter'||that.type==='heatmap') ? ((data[1])?(data[1][col]):'') : '';
                if (that.useCss === 'yes' &&
        ...
        drawXLine: function() {
            ...
            for (let i = 0; i < op.length; i++) {
                let font = op[i].font || '100 18px \'Arial\'';
                let value = op[i].val || 0;
                // useRow > value || 0
                if (this.type === 'line'||this.type === 'candle') {
                    if (op[i].useRow !== undefined) {
                        let useRow = op[i].useRow || ((op[i].useRow === 0) ? 0 : this.dataRowLen - 1);
                ...
                let lineColor = op[i].color || 'rgba(204,153,0,0.7)';
                let valueColor = op[i].vColor || lineColor;
                let lineWidth = op[i].width || 1;

                let fillOver = op[i].fillOver || null;
                let fillUnder = op[i].fillUnder || null;

                let xOffset = this.chartRight - (op[i].xOffset || 2);
                let yOffset = -(op[i].yOffset || 4);
                let val = this.unitH * value;
                // this.chartHeight * value / this.maxY; //parseInt(this.maxY/10, 10);
                let shdw = (this.shadows) ? this.shadows.xline || this.shadows.all || ['#444', 2, 2, 2] : '';

                let keep = op[i].keep || 'no';
                let ws = this.getWsById(this.id);// 現在のWebSocketオブジェクトを取得
                if (ws) {
                ...
                let top = (this.chartBottom - (val - this.minY * this.unitH)) || 0;
        ...
        _drawLine: function(op, func) {
            ...
            let lineWidthSet = this.util.setLineWidthSet(this, op);
            let colorSet = op.colorSet || this.colorSet;
            let shdw =
        (this.shadows) ? this.shadows.line || this.shadows.all || ['#222', 5, 5, 5] : '';
            for (let k = 0; k < this.dataRowLen; k++) {
                ...
                if (this.useShadow === 'yes') {if (lineWidthSet[k] > 1) this.drawShadow(shdw[0], shdw[1], shdw[2], shdw[3]);
}
                ...
                for (let l = 0; l < this.data[k].length; l++) {
                    let y = this.chartBottom - (this.data[k][l] - this.minY) * this.unitH;
                    if (this.type === 'line' || this.type === 'bezi2' || this.type === 'bezi') {
                        if (this.yScaleOrder === 'ASC') {
                            // lineだけY軸目盛昇順可能　yScaleOrder
      ...
      if(this.op.config.onend){
        ...
        _drawStackedArea: function(op, func) {
            ...
            let lineWidthSet = this.util.setLineWidthSet(this, op);
            let colorSet = op.colorSet || this.colorSet;
            let shdw = (this.shadows)?this.shadows.stackedarea||this.shadows.all||['#222', 5, 5, 5]:'';
            let wkData = [];
            let wkMaxYs = [];
...
if (lineWidthSet[k] > 1) this.drawShadow(shdw[0], shdw[1], shdw[2], shdw[3]);
        ...
        drawBar: function() {
            let that = this;
            let shdw = (this.shadows)?this.shadows.bar||this.shadows.all||['#222', 5, 5, 5]:'';
          ...
          + that.barGap * (that.dataRowLen-1);
            ...
            function okDrowBar() {
                ...
                _drowBar(function(that, k, l, x, y, yVal) {
                    let tyouseiY=(that.minY<0)?0:that.minY;
                    that.ctx.fillRect(
                        x, y,
                        that.barWidth,
                ...
                function _drowBar(func) {
                    that.ctx.save();
                    for (let k = 0; k < that.dataRowLen; k++) {
                        let x = that.chartLeft +that.barPadding;
                        that.ctx.beginPath();
                        that.ctx.fillStyle = that.colorSet[k];
                        if (that.useShadow === 'yes')that.drawShadow(shdw[0], shdw[1], shdw[2], shdw[3]);
                        for (let l = 0; l < that.data[k].length; l++) {
                            let tyouseiY=(that.minY<0)?that.minY:that.minY;
                            let y = that.chartBottom - (that.data[k][l]-tyouseiY)* that.unitH;
                            let yVal = that.data[k][l];
        ...
        drawStackedBar: function() {
            ...
            let that = this;
            let useSumLine = 'yes';
            let x = that.chartLeft +(that.barPadding||(((that.chartWidth/that.dataColLen)-that.barWidth)/2));
            let shdw =
        (this.shadows)?this.shadows.stacked||this.shadows.all||['#222', 5, -5, 5]:'';
            let zeroBase = (this.minY<0)? this.minY * this.unitH:0;

            // Cols
            for (let k = 0; k < that.dataColLen; k++) {
                let currntY = 0; // 積重ねた現在地
                let currntYUp = 0; // 積重ねた現在地
                let currntYDown = 0; // 積重ねた現在地

                // Rows
                for (let l = 0; l < that.dataRowLen; l++) {
                    let direction = (that.data[l][k]>=0)?1:-1;// 1===up -1===down ブロックの向き 値が-なら下向き
                    let blockHeight = (that.data[l][k]||0) * that.unitH;// 各ブロックの高さ
                    ...
                    let nextDirection =direction;// デフォルトdirection
                    // ブロックのオフセットY
                    var transOffsetY;
        ...
        drawCandle: function() {
            ...
            let candleLineWidth = this.op.config.candleLineWidth || this.gcf.candleLineWidth || 1;
            let candleBoxWidth = this.op.config.candleBoxWidth || this.gcf.candleBoxWidth || 10;
            ...
            let candleColorLine = this.op.config.candleColorLine || this.gcf.candleColorLine || '#888';
            let candleColorUp = this.op.config.candleColorUp || this.gcf.candleColorUp || 'deeppink';
            let candleColorDown = this.op.config.candleColorDown || this.gcf.candleColorDown || '#888';

            let x_line = that.chartLeft + (this.xGap/2);// lineのx位置
            let x_bar = that.chartLeft + (this.xGap/2) - (candleBoxWidth/2);// barのx位置

            this.ctx.save();

            for (let k = 0; k <= that.dataColLen; k++) {// date
                let _data = that.data;
                ...
                // draw line
                let y_line_hight = that.chartBottom - (hight - that.minY) * that.unitH;
                let y_line_low = that.chartBottom - (low - that.minY) * that.unitH;
                ...
                // draw box
                let y_bar_open = that.chartBottom - (barTop - that.minY||0) * this.unitH;
                let y_bar_close = Math.abs(upDown) * this.unitH;
        ...
        drawbeziLine: function() {
            ...
            let lineWidthSet = this.util.setLineWidthSet(this, this.op);
            let recx=0, recy=0;
            let shdw = (this.shadows)?this.shadows.bezi||this.shadows.all||['#222', 5, 5, 5]:'';
            for (let k = 0; k < this.dataRowLen; k++ ) {
                let x = this.chartLeft;
                x += this.xGap/2; // オフセット
                this.ctx.beginPath();
        ...
        drawbeziLine2: function() {
            let shdw = (this.shadows)?this.shadows.bezi2||this.shadows.all||['#222', 5, 5, 5]:'';
            ...
            for (let k = 0; k < this.dataRowLen; k++ ) {
                ...
                for (let l = 0; l < this.data[k].length; l++ ) {
                    let y = this.chartBottom - (this.data[k][l] - this.minY) * this.unitH;
                    let cpx = x+(this.xGap)/2;
                    let cpy = this.chartBottom
        ...
        drawHeatmap: function() {
            ...
            let it=this.coj[this.id];// current ccchart object

            it.hmBox = createHmDiv();// heatmap用のdiv hmBox
            it.hmCanvas = createHmCanvas(); // heatmap用のcanvas hmCanvas
            let ctx = it.hmCanvas.getContext('2d');

            // カレントccchart canvas
            let currCanvas = it.ids[it.id];

            // hmBoxにhmCanvasを挿入する
            it.hmBox.appendChild(it.hmCanvas);

            // グラデーション
            let _COLOR_GRADIENT = this.hm_grad || _GRADIENT;
            ...
            function createHmDiv() {
                ...
                if (it.hmBox=== null) {
                    ...
                    let currCanvas = it.canvas;// カレントccchart canvas
            ...
            function createHmCanvas() {
                ...
                if (it.hmCanvas=== null) {
                    ...
                    for (let i in it.canvas.style) {
                        let name = it.util.camelCase2cssName(i);
                        // 少し削る
                        ...
                        let val=it.util.getStyle(it.id, name);
                        if (val) {
        ...
        drawPie: function() {
            this.ctx.save();
            let that = this;
            let shdw = (this.shadows)?this.shadows.pie||this.shadows.all||['#444', 3, 3, 3]:'';
        ...
        this.op.config.colNameFont ||this.op.font || this.gcf.colNameFont || '100 18px \'Arial\'';
            let cnfs = this.util.getFontSizeFromStyle(colNameFont);
            let valFont = this.valFont =
        this.op.config.valFont || this.op.font || this.gcf.valFont || '100 12px \'Arial\'';
            let stfs = this.util.getFontSizeFromStyle(
                (this.op.config.subTitleFont || this.gcf.subTitleFont || '100 12px \'Arial\'')
            );
            let xOffset = this.op.config.pieXOffset || this.gcf.pieXOffset || 0;
            let yOffset = this.op.config.pieYOffset || this.gcf.pieYOffset || 0;
            let pieValPosOffset = this.op.config.pieValPosOffset || this.gcf.pieValPosOffset || 8;

            // pieHoleRadiusやpieRingWidthを指定しても自動調整されるので
            // 信用できないところが使いにくい

            let hfW = this.chartWidth/2; // チャートエリア幅 1/2
            let hfH = (this.chartHeight - stfs)/2; // チャートエリア高さ 1/2
            let ringWmin = 20;// リング幅の最小サイズ
            let holeRmin = 50;// ドーナツ穴の半径最小サイズ

            let holeR = this.pieHoleRadius; // ドーナツ穴の半径
            ...
            let radius = getRadius(holeR, ringW);
            ringW = Math.min(
                ((radius >= hfW)?(hfW - holeR):ringW),
                ((radius >= hfH)?(hfH - holeR):ringW)
            );
            radius = getRadius(holeR, ringW);
            let center = Math.min(this.chartHeight, this.chartWidth );
            let y = this.chartTop+this.chartHeight/2 + yOffset;
            let x = this.width/2 + xOffset;
            let colAry = [];
            let degAry = [];
            let perAry = [];
            let newDeg = 0, oldDeg = 0;
            let TORAD = 2*Math.PI/360;// deg to radian
            let pieDataLastIndex = pieDataIndex;// this.dataColLen

            this.drawXscale(this.chartLeft, 0);

            for (let k = pieDataIndex; k <= pieDataLastIndex; k++) {
                colAry[k]=degAry[k]=[]; deg = 0;
                for (let preCol = 0; preCol < this.dataRowLen; preCol++ ) {
                ...
                for (let l = 0; l < degAry[k].length; l++ ) {
                    newDeg += degAry[k][l];
                    let val = colAry[k][l];
                    _drawArc(
                        x, y, radius,
            ...
            function _drawArc(x, y, r, rStartAng, rEndAng, color, k, l, val) {
                ...
                // 値のXY
                let rad = rStartAng + (rEndAng - rStartAng)/2;
                let valX = x+(radius+ringW/2+pieValPosOffset) * Math.cos( rad );
                let valY = y+(radius+ringW/2+pieValPosOffset) * Math.sin( rad );
        ...
        flip: function(cvs, lists, flipDirection) {
            let that = this;
            let xbcss = this.util.xbcss;
            let direction = flipDirection?flipDirection:this.flipDirection;
            let scaleProp = (direction === 'x')? 'scaleX' : 'scaleY';
            let index = parseInt(this.canvas.getAttribute('data-flipIndex'), 10);
            ...
            function chg(cvs, next, stepMax) {
                ...
                this.flipID = setInterval(function() {
                    try {
                        ...
                        clearInterval(this.flipID); console.log('err')
                        ;
}
                }, interval);
            }
        ...
        flipX: function(cvs, lists) {
            this.flip(cvs, lists, 'x'); return this;
        },
        flipY: function(cvs, lists) {
            this.flip(cvs, lists, 'y'); return this;
        },
        ...
        ws: function(url, op) {
            ...
            let id = this.id;
            let uid = '-ccchart-ws-' + this.util.uuidv4();
            let w = this.wsuids[uid];
...
console.log('\n--------------------------------'
                    ...
                    if (op.autoReConnect) {if (that.wsReCnt['-ccchart-ws-'+id+'-'+url]===undefined) {
                        if (!that.coj[id].wsReConnecting)
                ...
                that.wsuids[uid].on('close', function() {
                    ...
                    if (that.wsDbg || that.wsInfo) {
                        if (that.coj[id].wsReConnecting) {
                            let cnt = that.wsReCnt['-ccchart-ws-'+id+'-'+url];
                            let maxcnt = op.maxReConnect;
                ...
                + '/' + op.maxReConnect;
                  ...
                  + uid);
                    ...
                    // 自動再接続 true の場合に再起動
                    if (op.autoReConnect){setTimeout(function() {
_autoReConnect(target)
;}, op.autoReConnectInterval);}
                });
            ...
            + '/' + maxcnt;
                ...
                let w = that.init(id, that.coj[id].op, that.coj[id].ondrew)
                    .ws(url, op)
                    .on('open', function() {
                        that.wsReCnt['-ccchart-ws-'+id+'-'+url] = 0;
                    })
...
console.log('\n================================'
            ...
            function _hbTimerFnc(target) {
                if (!op.useHb) return;
                // WebSocket用ハートビート設定
                let _MIN_HBINTERVAL = 5000;
                let _INI_HBINTERVAL = 60000;
                op.hbInterval =
              ...
              _INI_HBINTERVAL;
                target.on('message', function(msg) {
                    ...
                    if (msgs === target.op.hbStr) {
                        ...
                        if (that.wsDbg || that.wsInfo) {console.log('Heartbeat returned: RTT'
                            , target.op.RTT + 'ms'
                            , msgs, '#'+ id
                            , uid
                        );
        ...
        wsCloseById: function(id) {
            ...
            for (let i in this.wsuids) {
                let target = this.wsuids[i];
                if (!target) continue;
                if (target.op.id === id) {
                    this._wsOnClose(target
        ...
        wsCloseByUrl: function(url) {
            ...
            for (let i in this.wsuids) {
                let target = this.wsuids[i];
                let targetUrl = target.op.url.toLowerCase().replace(/\/$/, '');
                var url = url.toLowerCase().replace(/\/$/, '');
                if (!target) continue;
                if (targetUrl === url) {
                    this._wsOnClose(target
        ...
        wsClose: function(id, url) {
            ...
            if (!id) return;
            if (!url) return;
            let target = this.wsuids['-ccchart-ws-'+id+'-'+url];
            //  if (!target)return;
            console.log(id, url, this.wsuids['-ccchart-ws-'+id+'-'+url], 3333333333333333);
            target.on('close',
                function(e) {
                    if (that.wsDbg) {console.log('this closed was by the wsClose method: '+e.target.op.url);
}
      ...
      [
        ...
        [   600,  335,  584,  333,  457]
            ...
            oneColAtATime: function(msg) {
                ...
                let that = ccchart.coj[this.op.id];
                let opOrg = this.opOrg;

                if (that._wsThinout(this, this.wsThinOutInterval)) return;// ws受信データを間引く

                for (var i = 0; i < msgs.length; i++) {
                    let _title = (opOrg.data[i])?(opOrg.data[i][0]||''):'';
                    if (!that.op.data[i]) {
            ...
            someColsAtATime: function(msg) {
                ...
                for (let j = 0; j < msgs.length; j++) {
                    for (let i = 0; i < msgs[j].length; i++) {
                        let rowTitle = that.op.data[i].shift(); // 先頭を行タイトルとして削除
            ...
            mkOpData: function(addAry, opData, maxWsColLen) {
                ...
                for (let i = 0; i < addAry.length; i++) {
                    if (!opData[i]) continue;
                    let rowTitle = opData[i].shift(); // 先頭を行タイトルとして削除
        ...
        move1: function(cvs, config, dataAry, delay) {
            ...
            let that = this;
            let dly = (delay!==undefined)?delay:100;
            let cnfIsAry = this.util.isArray(config);
            let i = 0;
        ...
        move2: function(colDataAry, delay) {
            ...
            function draw(that) {
                let data =JSON.parse(that.moveStack.shift());
                that.init({
                    'config': cnf,
                    'data': data,
                });
        ...
        drawMemo: function(op) {
            ...
            let left = op.left || 0;
            let top = op.top || 0;
            let lineTo = op.lineTo || false;
            let lineTo2 = op.lineTo2 || false;
            if (!(val && ((left && top) || lineTo || lineTo2))) return this;
            let font = op.font || '100 14px \'Arial\'';
            let textAlign = op.align || 'left';
            let fillStyle = op.color || this.textColor ||this.textColors.all|| this.textColors.memo || '#ccc';
            let opacity = op.opacity || this.textOpacity || 1;
            ...
            function _lineTo(row, col) {
                // line
                let x = that.chartLeft + that.xGap / 2 + that.xGap * col;
                let y = that.chartBottom - ( that.data[row][col] - that.minY ) * that.unitH;
                that.ctx.save();
                that.ctx.beginPath();
            ...
            function _getNum(id, prop, val) {
                for (let i=0; i<that.coj[id][prop].length; i++) {
        ...
        add: function(op, callback) {
            let initArgs = this.util.deepJSONCopy({id: this.id, op: this.coj[this.id].op, callback: this.coj[this.id].callback});// init側
            let addArgs = this.util.deepJSONCopy({id: this.id, op: op, callback: callback});// addかafter側
        ...
        before: function(op, callback) {
            let initArgs = this.util.deepJSONCopy({id: this.id, op: this.coj[this.id].op, callback: this.coj[this.id].callback});// init側
            let beforeArgs = this.util.deepJSONCopy({id: this.id, op: op, callback: callback});// before側
        ...
        line: function(op) {
            this.drawLine(op); return this;
        },
        markers: function(op) {
            this.drawMarkers(op); return this;
        },
        memo: function(op) {
            this.drawMemo(op); return this;
        },
        ...
        moduleExtend: function(oj, prop, name, update) {
            ...
            if (!prop['aboutThis_module']) {prop['aboutThis_module'] = {
                name: name,
            };
}
            for (let i in prop) {
        ...
        loadBranch: function(op, callback) {
            ...
            let isStrOp = (typeof op === 'string');
            let isStrOpCnf = (typeof op.config === 'string');
            let isStrOpData = (typeof op.data === 'string');
            if (isStrOp) {
        ...
        util: {
            ...
            setConfigNum: function(it, propName, configVal, gfcVal, defaultVal) {
                ...
                function _regex(val) {
                    ...
                    let _val=parseFloat((''+val).replace(/[^-{0,1}[0-9]\.]/g, ''));
            ...
            hm: {
                coloring: function(grayGrad, colorGrad) {
                    let grayGradData = grayGrad.data;
                    let colorGradData = colorGrad.data;
                    let len = grayGradData.length;
                    let offset;
                ...
                mkGrayImgData: function(grad, that, ctx, x, y, innerCircle, outerCircle) {
                    let gradient = ctx.createRadialGradient(x, y, innerCircle, x, y, outerCircle);
                    for (let i in grad) {
                ...
                mkColorImgData: function(grad, it) {
                    ...
                    let ctx = it._hmColorImgDataCanvas.getContext('2d');
            ...
            setNumberOfConfigVal: function(it, prop, def) {// e.g. axisYSkipWidth...
                ...
                let num = (it.op.config[prop]!==undefined)?it.op.config[prop]:
          ...
          (titleTextAlign === 'center')?(0+offsetX):
            ...
            getAxisIndex: function(axisStr, pos) {
                ...
                let posStr = (axisStr === 'axisYs')?'left':'top';
                let data = ccchart[axisStr];
                for (let i = 0; i < data.length; i++) {
                    if (axisStr === 'axisYs') {if(pos < data[i][posStr]) {
return [i, pos, data[i][posStr]];
}}
                    if (axisStr === 'axisXs') {if (pos > data[i][posStr]) {
return [i, pos, data[i][posStr]];
}}
                }
            },
            getValWidth: function(it, val, fontSize) {
                ...
                if (typeof val === 'number') {
                    let len = Math.floor(Math.log(val)/Math.log(10))+1;
                    return len * fontSize;
            ...
            cnfExtend: function(baseCnf, newCnf) {
                ...
                let base = ccchart.util.deepJSONCopy(baseCnf);
            ...
            round00: function(val, n) {
                ...
                let _digit = Math.pow(10, n);
                let _val = parseFloat(val);
                let str = '' + Math.round(_val * _digit)/_digit;
            ...
            _preGetMinMax: function(that) {
                // 全データを連結した配列を返す
                let _ary = [];
                for (let i = 0; i < that.dataRowLen; i++) {
                    let _aryR=[];
                    for (let j = 0; j < that.data[i].length; j++) {
                        if (
                            !(that.data[i][j]===null ||
            ...
            mkPercentVal: function(it, val, k, l) {
                let sumCols = it.util.getSumCols(it);

                let percent = Math.round( val*100/sumCols[k], 10) + '%';
                if (it.type === 'stacked') {
            ...
            getMaxSum: function(that) {
                ...
                let _ary = [];
                let _sum = 0;
                for (let i = 0; i < that.dataColLen; i++) {
                    for (let j = 0; j < that.dataRowLen; j++) {
                        if (that.type==='stacked') {
                            // only plus data
                            if (that.data[j][i]>0) {
            ...
            isMobile: function() {
                ...
                let ua = navigator.userAgent.toLowerCase();
                let match = /(android|iphone|ipad|iemobile)/.exec(ua);
          ...
          +';';
            ...
            getFontSizeFromStyle: function(fontStyle) {
                let fontSize = (fontStyle).split(' ')[1]||0;
...
(function(window) {
    return {
        ...
        useCssSetting: function() {
            ...
            let top = getNum('top');
            let left = getNum('left');
            let marginTop = getNum('margin-top');
            let marginLeft = getNum('margin-left');
            let paddingTop = getNum('padding-top');
            let paddingLeft = getNum('padding-left');
            let borderTopWidth = getNum('border-top-width');
            let borderLeftWidth = getNum('border-left-width');
            let currZindex = getNum('z-index');
        ...
        document.querySelector('#-ccchart-css-hybrid');
            if (this.hybridBox === null) {
                let hybridBox = document.createElement('div');
        ...
        document.querySelector('#-ccchart-css-group-' + id);
            if (!this.cssgs[id]) {
                ...
                let cssGrp = document.createElement('div');
            ...
            if (position === 'fixed') {
                let fixedZ = this.canvasZindex + 2;
            ...
            function addToolTip(that) {
                ...
                if (!that.cssTooltips[id]) {
                    let csstooltip = document.createElement('div');
              ...
              that.cssgs[id].appendChild(csstooltip);
                        try {
                            let _tooltipop = that.cssTooltip;
                            if (_tooltipop) {
            ...
            function getHybridCss() {
                ...
                for (let i=0; i<document.styleSheets.length; i++) {
                    if (document.styleSheets[i].title==='-ccchart-css-styles') {sheet = document.styleSheets[i];
} break;
                }
                return sheet;
            }
        ...
        adjustCss: function(type) {
            let that = this; // console.log(that.id, that.adjustCss.caller);
            ...
            for (var i in this.coj) {
                ...
                getPos(i, el, level, 10, 0, 0,
                    function(numAry) {
                        let topOff = that.coj[i]['adjustTop'];
                        let leftOff = that.coj[i]['adjustLeft'];
                        let top = topOff + numAry[0]||0;
                        let left = leftOff + numAry[1]||0;
            ...
            function getPos(id, el, level, deep, numT, numL, fn) {
                let current = el.nodeName.toUpperCase();
                if (current === 'BODY') return;
                if (!deep) var deep = 5;
                let css = el.currentStyle || document.defaultView.getComputedStyle(el, null);
                let parentoffNode = el.offsetParent||el.parentNode;
            ...
            that.coj[id].targetPos==='absolute'
              ...
              current === 'TABLE'
                            ...
                            let borderTopW = parseInt(css['borderTopWidth'], 10);
                            let borderLeftW = parseInt(css['borderLeftWidth'], 10);
                            let syusei = that.ua.match(/firefox/)?2:1;
        ...
        _css_arc: function(op) {
            ...
            let x = op.x || 0;
            let y = op.y || 0;
            let radius = op.radius || 5;
            let borderWidth = this.borderWidth || 3;
            let colorIndex = ((this.type==='scatter'||this.type==='heatmap')?op.colorIndex:op.row)||0;
            let borderColor = op.borderColor || op.colorSet[colorIndex] || op.colorSet[colorIndex];
            let bgcolor = op.bgColor || op.colorSet[colorIndex] || op.colorSet[colorIndex];
            let tipBgcolor = bgcolor;
            ...
            let etc = (op.etcStyle?op.etcStyle:'');
            let el = document.createElement('div');
            if (borderWidth > radius)radius = borderWidth;// ringのborder幅が半径を超えたら半径を拡大

            let dataX = (this.type==='scatter'||this.type==='heatmap')?(this.rowNames[0]):this.colNames[op.col];
            let dataY = (this.type==='scatter'||this.type==='heatmap')?(this.rowNames[1]):this.rowNames[op.row];

            let unit = that.coj[id].unit;
            if (typeof unit==='string') {
                unit = unit;
            } else if (typeof unit==='object') {
                unit = unit.unit;
        ...
        (this.dataColLen-1)===op.col
          ...
          + this.pfx['transition'] +': background-color 200ms linear';
            ...
            // マーカーをグループへ登録
            let arc = this.cssgs[id].appendChild(el);
            el = null;

            function showTooltip(id, e, op) {
                that.csstoolRock = !!that.csstoolRock;
                let rowNum = e.target.getAttribute('data-row');
                let rowName = e.target.getAttribute('data-y');
                let colName = e.target.getAttribute('data-x');
                let scatterXData = e.target.getAttribute('data-scatter-x');
                let scatterYData = e.target.getAttribute('data-scatter-y');
                let colname = e.target.getAttribute('data-colname');
                let colnamestitle = e.target.getAttribute('data-colnamestitle');
                let data = e.target.getAttribute('data-data');
                let percent = (that.percentVal === 'yes')?'( ' + e.target.getAttribute('data-percent') + ' )':'';
                var unit = e.target.getAttribute('data-unit');
                let bgcolor = e.target.getAttribute('data-bg');
                let colNamesTitle = (this.type==='scatter'||this.type==='heatmap')?
                ...
                let toolTemp = !that.coj[id].tmpToolTip?'':
          ...
          document
              ...
              .getComputedStyle(that.cssTooltips[id], null);
                setTimeout(function() {
                    ...
                    let height = parseFloat(computedStyle.getPropertyValue('height'), 10)||0;
                    let borderTop = parseFloat(computedStyle.getPropertyValue('border-top'), 10)||0;
                    let borderBottom = parseFloat(computedStyle.getPropertyValue('border-bottom'), 10)||0;
                    let paddingTop = parseFloat(computedStyle.getPropertyValue('padding-top'), 10)||0;
                    let paddingBottom = parseFloat(computedStyle.getPropertyValue('padding-bottom'), 10)||0;
                    computedStyle = null;
...
that.cssTooltips[id].innerHTML = toolTemp || htm;
          ...
          +'}',
            ...
            if (this.useToolTip==='yes') {
                // Cross Event type
                let mbl = this.util.isMobile();
                let over = (mbl)?'touchstart':'mouseover';
                let out = (mbl)?'touchend':'mouseout';

                // 触った時のイベント登録 1)触ったら現れ、離れるまで消えない
                arc.addEventListener(over, function(e) {
                    showTooltip(id, e, op);
                } );
        ...
        deleteCssRule: function(selectorText) {
            for (let i=0; i<this.hybridCss.cssRules.length; i++) {
...
window.ccchart.m.Theme = {
    ...
    set: function(color) {
        let _theme = ccchart.util.cnfExtend( this.white);
